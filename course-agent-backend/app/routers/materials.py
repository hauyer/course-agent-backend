...
def process_material_background(*, material_id: int, user_id: int) -> None:
    """在请求返回后完成解析、分块与向量化，避免上传接口长时间等待。"""
    db = SessionLocal()
    try:
        ...
        if material is None:
            return
        mark_material_processing(db=db, material=material)
        try:
            raw_text = parse_document(Path(material.file_path))
            ...
            if not chunks:
                ...
                return
            replace_material_chunks(db=db, material=material, chunks=chunks)
    ...
    index_material_vectors_background(material_id=material_id, user_id=user_id)
...
def delete_all_user_materials_api(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """删除当前用户上传的全部课程资料、分块、向量与磁盘文件。"""
    materials = (
        db.query(Material)
    ...
    if not materials:
        return {"message": "没有需要删除的课程资料", "deleted_count": 0}
    ...
    user_upload_directory = (UPLOAD_ROOT / str(current_user.id)).resolve()
    try:
        user_upload_directory.relative_to(upload_root)
        if user_upload_directory.is_dir():
            shutil.rmtree(user_upload_directory)
    except ValueError:
        pass
