...
@router.get(
    ...
    summary="读取当前用户的笔记集成配置",
)
def get_integration_config_api(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    config = get_integration_config(db, user_id=current_user.id)
    return serialize_integration_config(config)


@router.put(
    "/integrations/config",
    response_model=IntegrationConfigResponse,
    summary="保存当前用户的笔记集成配置",
)
def update_integration_config_api(
    config_in: IntegrationConfigUpdate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    config = save_integration_config(
        db,
...
def test_obsidian_connection_api(
    ...
    try:
        config = get_integration_config(db, user_id=current_user.id)
        with obsidian_config_context(require_obsidian_runtime(config)):
...
def test_notion_connection_api(
    ...
    try:
        config = get_integration_config(db, user_id=current_user.id)
        with notion_config_context(require_notion_runtime(config)):
