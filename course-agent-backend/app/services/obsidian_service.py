...
_obsidian_runtime_config: ContextVar[tuple[str, str] | None] = ContextVar(
    "obsidian_runtime_config", default=None
)


@contextmanager
def obsidian_config_context(config: tuple[str, str]):
    """Bind a per-user Vault path and base folder for one operation."""
    token = _obsidian_runtime_config.set(config)
    try:
        yield
    finally:
        _obsidian_runtime_config.reset(token)
...
def get_obsidian_vault_path() -> Path:
    ...
    runtime = _obsidian_runtime_config.get()
    raw_path = runtime[0] if runtime is not None else os.getenv("OBSIDIAN_VAULT_PATH")
...
def get_obsidian_base_folder() -> str:
    ...
    raw_folder = (
        ...
        else os.getenv("OBSIDIAN_BASE_FOLDER", "课程学习助手")
    )
