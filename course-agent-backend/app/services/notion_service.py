...
_notion_runtime_config: ContextVar[tuple[str, str, str, float] | None] = ContextVar(
    "notion_runtime_config", default=None
)


@contextmanager
def notion_config_context(config: tuple[str, str, str, float]):
    """Bind a per-user Notion configuration for one test or sync operation."""
    token = _notion_runtime_config.set(config)
    try:
        yield
    finally:
        _notion_runtime_config.reset(token)
...
def get_notion_config() -> tuple[str, str, str, float]:
    """读取并验证 Notion 配置。"""
    runtime = _notion_runtime_config.get()
    if runtime is not None:
        token, raw_parent_id, api_version, timeout_seconds = runtime
        return token, normalize_notion_id(raw_parent_id), api_version, timeout_seconds
