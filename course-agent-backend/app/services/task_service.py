...
def _sync_completion_record(
    db: Session,
    *,
    task: Task,
    previous_status: str,
) -> None:
    """Keep task-generated study time aligned with the completion state."""
    ...
    existing = (
        ...
        .filter(
            LearningRecord.user_id == task.user_id,
            LearningRecord.task_id == task.id,
            LearningRecord.source == "task",
        )
        .all()
    )
    ...
    if task.course_id is None or existing:
        return
    ...
    record = LearningRecord(
        ...
        source="task",
        content_summary=f"完成任务：{task.title}",
    )
    db.add(record)

    # Reuse the course progress invariant without introducing a module cycle
    # during import time.
    from app.services.learning_service import _get_or_create_progress
    ...
    if progress.started_at is None:
        progress.started_at = studied_at
